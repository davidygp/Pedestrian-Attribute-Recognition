...
def dpn68b(pretrained='imagenet+5k'):
    model = DPN(
        small=True, num_init_features=10, k_r=128, groups=32,
...
def dpn92(pretrained='imagenet+5k'):
    model = DPN(
...
def dpn131(pretrained='imagenet'):
    model = DPN(
...
def dpn107(pretrained='imagenet+5k'):
    model = DPN(
